...
- Inbound messages are stored in the data database while a session is running.
- `GET /api/sessions/:sessionId/conversations` — conversation list (includes `unreadCount` / `hasUnread`)
- `GET /api/inbox/conversations` — unified list across all accessible sessions
- `PATCH /api/sessions/:sessionId/conversations/:chatId/read` — mark read (dashboard cursor + WhatsApp `sendSeen`)
- `PATCH /api/inbox/conversations/read` — mark read with body `{ sessionId, chatId }`
- `GET /api/sessions/:sessionId/messages?chatId=...` — thread messages
- `GET /api/sessions/:sessionId/chats` — WhatsApp chat names (requires started session)
- Dashboard: **Inbox** at `http://localhost:2886/inbox` (unread badges, bold rows, mark-read on open)

**Unread behavior:** Counts are incoming messages after the last read cursor in `inbox_thread_reads`. When the session is `ready`, counts also merge with live `unreadCount` from WhatsApp so phone-side unread can appear before messages are persisted. Opening a thread in the dashboard marks it read and sends read receipts to the customer (same as reading in WhatsApp). Reading on the linked phone clears unread via the engine `unread_count` event.
