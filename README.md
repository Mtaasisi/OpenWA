...
### Inbox (conversations & message history)

Inbound messages are persisted automatically when a session is running. Use these endpoints to build an inbox UI or integrate with your app:

```bash
# List conversations for a session (aggregated from stored messages)
curl -H "X-API-Key: YOUR_API_KEY" \
  http://localhost:2785/api/sessions/{sessionId}/conversations

# Message thread for a chat
curl -H "X-API-Key: YOUR_API_KEY" \
  "http://localhost:2785/api/sessions/{sessionId}/messages?chatId=628123456789@c.us&limit=50"

# Optional: chat display names from WhatsApp (session must be started)
curl -H "X-API-Key: YOUR_API_KEY" \
  http://localhost:2785/api/sessions/{sessionId}/chats
```

The dashboard **Inbox** page (`http://localhost:2886/inbox`) provides a session-scoped UI with live updates via WebSocket.
